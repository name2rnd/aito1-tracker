...
) RETURNING id, workspace_id, title, description, icon, status, lead_type, lead_id, created_at, updated_at, priority, position
`
...
SELECT id, workspace_id, title, description, icon, status, lead_type, lead_id, created_at, updated_at, priority, position FROM project
WHERE id = $1
`
...
const updateProjectPositionInWorkspace = `-- name: UpdateProjectPositionInWorkspace :one
UPDATE project SET
    position = $3,
    updated_at = now()
WHERE id = $1 AND workspace_id = $2
...
type UpdateProjectPositionInWorkspaceParams struct {
	ID          pgtype.UUID `json:"id"`
	WorkspaceID pgtype.UUID `json:"workspace_id"`
	Position    int32       `json:"position"`
}

func (q *Queries) UpdateProjectPositionInWorkspace(ctx context.Context, arg UpdateProjectPositionInWorkspaceParams) (Project, error) {
	row := q.db.QueryRow(ctx, updateProjectPositionInWorkspace, arg.ID, arg.WorkspaceID, arg.Position)
	var i Project
	err := row.Scan(
		&i.ID,
		&i.WorkspaceID,
